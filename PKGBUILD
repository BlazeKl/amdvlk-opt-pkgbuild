...
package() {
  install -m755 -d "${pkgdir}"/opt/amdvlk/usr/lib
  install -m755 -d "${pkgdir}"/opt/amdvlk/usr/share/vulkan/icd.d
  install -m755 -d "${pkgdir}"/opt/amdvlk/usr/share/licenses/amdvlk-git
  install -m755 -d "${pkgdir}"/opt/amdvlk/etc/amd

  install xgl/builds/Release64/icd/amdvlk64.so "${pkgdir}"/opt/amdvlk/usr/lib/
  install AMDVLK/json/Redhat/amd_icd64.json "${pkgdir}"/opt/amdvlk/usr/share/vulkan/icd.d/
  install AMDVLK/LICENSE.txt "${pkgdir}"/opt/amdvlk/usr/share/licenses/amdvlk-git/

  sed -i "s/\/lib64/\/lib/g" "${pkgdir}"/opt/amdvlk/usr/share/vulkan/icd.d/amd_icd64.json
}
